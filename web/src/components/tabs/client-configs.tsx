...
import { ClaudeAI, Cursor, Gemini, JetBrains, OpenAI, OpenCode, VisualStudioCode, Zed } from "@/components/tabs/icons"
  ...
  const handleCopy = useCallback(async () => {
    try {
      if (navigator.clipboard?.writeText) {
        ...
        // Fallback for non-secure contexts (e.g. HTTP on K8s NodePort)
...
interface ClientConfigsProps {
  serverName: string
}

export function ClientConfigs({ serverName }: ClientConfigsProps) {
  ...
  const safeName = serverName.replace(/[^a-zA-Z0-9-]/g, '-').toLowerCase()
  ...
  "mcpServers": {
    ...
    },
    {
      ...
      icon: <ClaudeAI className="h-6 w-6" />,
      config: `claude mcp add ${safeName} --transport http ${serverUrl}/mcp`,
    },
    {
      title: "Cursor IDE",
  ...
  "mcpServers": {
    ...
    },
    {
      ...
      config: `codex mcp add ${safeName} --url ${serverUrl}/mcp`,
    },
    {
  ...
  "mcpServers": {
    ...
    },
    {
      ...
      config: `gemini mcp add ${safeName} --transport http ${serverUrl}/mcp`,
    },
    {
      title: "OpenCode",
